...
export default function LiveCheatMonitor() {
  ...
  // THE FIX: Normalize the URL code so it perfectly matches the database (Uppercase)
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        console.log(`📡 Fetching Exam & Students for: [${examCode}]`);

        // A. Get Exam Info
        const { data: examData, error: examErr } = await supabase
          .from("exams")
          .select("*")
          .ilike("code", `%${examCode}%`) // The % signs ignore hidden spaces!
          .single();
        ...
        // B. THE BULLETPROOF STUDENT FETCH
        const { data: studentData, error: studentErr } = await supabase
          .from("students")
          .select("*") // Fetch EVERYTHING to prevent missing-column crashes
          .ilike("exam_code", `%${examCode}%`) // Ignore hidden spaces
          .order("created_at", { ascending: false });

        console.log("👨‍🎓 Supabase Student Response:", studentData);

        if (studentErr) {
          console.error("❌ Supabase Error:", studentErr.message);
        }

        if (studentData && studentData.length > 0) {
          // Force TypeScript to accept the data format
          setStudents(studentData as unknown as Student[]);
        } else {
          console.log("⚠️ Supabase returned an empty array for students.");
        }
      } catch (err: unknown) {
